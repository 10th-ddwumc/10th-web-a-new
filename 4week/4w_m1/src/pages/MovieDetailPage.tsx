...
const MovieDetailPage = () => {
    ...
    // 기존에는 useState + useEffect + movieApi.get()으로 직접 상세정보를 가져왔음
    // 이제는 커스텀 훅이 data / isPending / isError를 대신 관리함
    ...
    } = useCustomFetch<MovieDetail>(`/${movieId}?language=ko-KR`);

    // 출연진 정보도 같은 방식으로 커스텀 훅 사용
    const {
        data: credits,
        isPending: isCreditsPending,
        isError: isCreditsError
    } = useCustomFetch<CreditResponse>(`/${movieId}/credits?language=ko-KR`);

    // 상세정보 요청 + 출연진 요청 둘 중 하나라도 로딩 중이면 로딩 처리
    const isPending = isMoviePending || isCreditsPending;

    // 둘 중 하나라도 에러가 나면 에러 처리
    ...
    /*useEffect(() => {
        const fetchMovieData = async () => {
            ...
            try {
                ...
                // 피드백 이전 : 영화 상세 정보와 출연진 정보를 각각 따로 API 호출
                // const [detailRes, creditRes] = await Promise.all([
    ...
    if (isError || !movie || !credits) return <div className='text-center text-white mt-10'>에러가 발생했습니다.</div>;
