...
const Mypage = () => {
    ...
    onMutate: async (newUserData: any) => {
        await queryClient.cancelQueries({ queryKey: ["userMe"] });

        const previousUserData = queryClient.getQueryData<ResponseMyinfoDto>(["userMe"]);
        queryClient.setQueryData<ResponseMyinfoDto>(["userMe"], (old) => {
            if (!old) return old;
            return {
                ...old,
                data: {
    ...
    onError: (err, newUserData, context) => {
        if (context?.previousUserData) {
            queryClient.setQueryData(["userMe"], context.previousUserData);
            setData(context.previousUserData);
        }
        alert("수정에 실패했습니다.");
    },
