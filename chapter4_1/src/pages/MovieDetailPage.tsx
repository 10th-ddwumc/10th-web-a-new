...
const MovieDetailPage = () => {
    ...
    const { data: movie, isLoading: isMoviePending, isError: isMovieError } = 
        useCustomFetch<MovieDetail>(`/movie/${movieId}?language=ko-KR`);

    const { data: credits, isLoading: isCreditsPending, isError: isCreditsError } = 
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-6 pb-4">
                    {credits?.cast?.slice(0, 10).map(actor => (
                        <div key={actor.id} className="flex flex-col items-center min-w-20">
                            <img
                                src={actor.profile_path ? `${IMG_URL}${actor.profile_path}` : '/profile.jpg'}
