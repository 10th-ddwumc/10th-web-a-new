...
const LpDetailPage = () => {
    ...
    const { mutate: toggleLike } = useMutation({
        mutationFn: () => axiosInstance.post(`v1/lps/${lpid}/likes`),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["lp", lpid] });
        }
    });
    ...
    return (
        <div className="max-w-5xl mx-auto p-6 text-white">
            <div className="flex justify-between items-center mb-6">
                ...
                {user && lp && String(user.id) === String(lp.authorId) && (
            ...
            <div className="flex flex-col md:flex-row gap-10 mb-12">
                ...
                <div className="w-full md:w-1/2 py-4 flex flex-col justify-between">
                    ...
                    <button 
                        onClick={() => toggleLike()}
                        className={`mt-8 self-start px-8 py-3 rounded-full font-bold transition-all ${lp.isLiked ? 'bg-pink-600' : 'bg-zinc-800 hover:bg-zinc-700'}`}
                    >
                                ...
                                <div key={comment.id} className="flex gap-4 group">
                                    ...
                                    <div className="flex-1 border-b border-white/5 pb-8 group-last:border-none">
                                        <div className="flex items-center justify-between mb-3">
                                            ...
                                            {user && Number(user.id) === Number(comment.authorId) && (
                                                <div className="flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <button onClick={() => { setEditingId(comment.id); setEditContent(comment.content); }} className="text-xs text-zinc-500 hover:text-white">수정</button>
                                                    <button onClick={() => setModalConfig({ open: true, type: "COMMENT", id: comment.id })} className="text-xs text-zinc-500 hover:text-red-500">삭제</button>
                                                </div>
                                            )}
                                        </div>
